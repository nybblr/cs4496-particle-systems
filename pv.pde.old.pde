...
// AUTHOR:        Prof Jarek Rossignac, with edits by Jonathan Martin
...
vec V(vec U,vec V) {return new vec(U.x+V.x,U.y+V.y); };
...
// Rigid body -- Jonathan Martin
float x(pt A, pt B, pt P) { // get the x component of P relative to the line AB
  // AB . AP / AB . AB
  float x = dot(V(A, P), V(A, B)) / dot(V(A, B), V(A, B));
  //println("Line "+A+B+" has x="+x+" for point "+P);
  return x;
}
float y(pt A, pt B, pt P) { // get the y component of P relative to the line AB
  // AB x AP / AB . AB
  float y = det(V(A, P), V(A, B)) / dot(V(A, B), V(A, B));
  //println("Line "+A+B+" has y="+y+" for point "+P);
  return y;
}
pt P(float x, float y, pt X, pt Y) {
  // A+xAB+yR(AB)
  // First make a copy of the points
  pt A = new pt(X);
  pt B = new pt(Y);

  vec xAB = V(A, B).scaleBy(x);
  vec yAB = R(V(A, B)).scaleBy(y);
  return A.add(xAB).add(yAB);
}
pt transform(pt A, pt B, pt P, pt C, pt D) {
  return P(x(A, B, P), y(A, B, P), C, D);
}
