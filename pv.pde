...
// Changes and additions by Jonathan Martin
...
class vec { float x=0,y=0,z=0;
   ...
   vec divideBy(float f) {x/=f; y/=f; z/=f; return this;};
...
float dot2(vec U, vec V) {return U.x*V.x+U.y*V.y; }                                                     // dot(U,V): U*V (dot product U*V)
float det2(vec U, vec V) {return dot2(U,R2(V)); }                                                         // det | U V | = scalar cross UxV
vec R2(vec V) {return new vec(-V.y,V.x, V.z);};                                                             // V turned right 90 degrees (as seen on screen)


// ===== point class
class pt { float x=0,y=0,z=0;
  pt () {};
  pt (pt A) {x = A.x; y = A.y; z = A.z;}
  ...
  boolean equal(pt P) { return x==P.x && y==P.y && z==P.z; }

  pt rotate(float x, float y, float z, pt C) { // rotate around X, Y, and Z axis at center c
    pt rot = this;
    rot = R(rot, V(1, 0, 0), C, x);
    rot = R(rot, V(0, 1, 0), C, y);
    rot = R(rot, V(0, 0, 1), C, z);
...
pt R(pt P, vec V, pt A, float a) { // Rotate P around axis V by a
  // Find closest point along axis
  pt C = P(A).add( V( d(U(V), V(A, P)), U(V) ) );
  vec I = U(V(C, P)); // vector from closest point on line to P
  vec J = U(N(V, I)); // orthonormal vector to I and V, such that the right hand rule is preserved.
  return R(P, a, I, J, C);
}
...
// Subdivision - Jonathan Martin
pt Subdivide(pt A, pt B, pt C, pt D, float s) {
  // the endpoints are allowed to be null
  if (A == null && D == null)
    return P(B, C);
  else if (D == null)
    return Parabola(A, B, C, s+0.25);
  else if (A == null)
    return Parabola(B, C, D, s-0.25);
  else
    return P(Parabola(A, B, C, s+0.25), Parabola(B, C, D, s-0.25));
}

pt Subdivide(pt A, pt B, pt C, pt D) {
  return Subdivide(A, B, C, D, 0.5);
}
